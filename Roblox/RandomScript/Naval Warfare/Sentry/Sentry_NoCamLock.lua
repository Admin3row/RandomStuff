-- AutoLock v1.39 No Camera Lock Version
...
local HIT_SOUND_ID = "rbxassetid://4329397755"
local HIT_VOLUME = 1.1
local HIT_INDICATOR_DURATION = 0.1
...
-- ensure we have a function to create/reparent hit indicator to the SAME parent as the game's Aim
-- create / refresh hit indicator and immediately parent to our persistent GUI
...
-- === TETHER VISUAL (paste near top with other locals) ===
local _TETHER = {}

local function createTether()
    if _TETHER.beam then return end

    local p0 = Instance.new("Part")
    p0.Name = "TetherOrigin_Client"
    ...
    a0.Name = "TetherAttach0"
    a0.Parent = p0

    local p1 = Instance.new("Part")
    p1.Name = "TetherEnd_Client"
    ...
    a1.Name = "TetherAttach1"
    a1.Parent = p1

    local beam = Instance.new("Beam")
    beam.Name = "TetherBeam_Client"
    beam.Attachment0 = a0
    beam.Attachment1 = a1
    ...
    _TETHER = { p0 = p0, p1 = p1, a0 = a0, a1 = a1, beam = beam }
end

local function destroyTether()
    if _TETHER.p0 then pcall(function() _TETHER.p0:Destroy() end) end
    if _TETHER.p1 then pcall(function() _TETHER.p1:Destroy() end) end
    _TETHER = {}
end

local function updateTether(originCFrame, aimVector, show, dist, currentMode)
    if not _TETHER.beam then createTether() end
    if not _TETHER.beam then return end

    -- position origin/end parts so Beam stretches between them
    pcall(function()
        _TETHER.p0.CFrame = CFrame.new(originCFrame.Position)
        _TETHER.p1.CFrame = CFrame.new(aimVector)
    end)

    -- enable/disable beam
    _TETHER.beam.Enabled = show and true or false

    -- optional: color by distance/mode if colorForDistance exists
    if show and dist and currentMode then
        local ok, colorOrTier = pcall(function() return colorForDistance(dist, currentMode) end)
        if ok and type(colorOrTier) == "userdata" then
            _TETHER.beam.Color = ColorSequence.new(colorOrTier)
        elseif ok and type(colorOrTier) == "table" and type(colorOrTier[1]) == "userdata" then
            _TETHER.beam.Color = ColorSequence.new(colorOrTier[1])
        end
    end
end
...
local function stringContainsTokenInsensitive(str, tokens)
    ...
    for _, tok in ipairs(tokens) do
        if string.find(lower, tok, 1, true) then
            return true
        end
    end
    return false
...
local function isPlayerInPlane(plr)
    ...
    nm = (ok and nm) or ""
    if PLANE_EXACT[nm] then
        return true, vehicle, true
    end
    if stringContainsTokenInsensitive(nm, PLANE_TOKENS) then
    ...
    if okType and tv and tv:IsA("StringValue") then
        ...
        if PLANE_EXACT[v] then
            return true, vehicle, true
        end
        if stringContainsTokenInsensitive(v, PLANE_TOKENS) then
            return true, vehicle, false
        end
    end
    if vehicle.GetAttribute then
        ...
        if okAttr and type(av) == "string" then
            if PLANE_EXACT[av] then
                return true, vehicle, true
            end
            if stringContainsTokenInsensitive(av, PLANE_TOKENS) then
                return true, vehicle, false
            end
        end
    end
...
local function isShipExact(vehicle)
    ...
    local ok, nm = pcall(function() return tostring(vehicle.Name or "") end)
    nm = (ok and nm) or ""
    if SHIP_EXACT[nm] then
        return true
    end
    local okType, tv = pcall(function() return vehicle:FindFirstChild("VehicleType") end)
    if okType and tv and tv:IsA("StringValue") then
        if SHIP_EXACT[tostring(tv.Value or "")] then
            return true
        end
    end
    if vehicle.GetAttribute then
        local okAttr, av = pcall(function() return vehicle:GetAttribute("VehicleType") end)
        if okAttr and type(av) == "string" and SHIP_EXACT[av] then
            return true
        end
    end
    return false
            ...
            if ch then
                ...
                if hum and hum.Health > 0 then
                    ...
                    if headPos then
                        ...
                        if inPlane then
                            -- If inPlane true, treat as plane. But avoid treating ships/submarines as infantry:
                            -- If the detected vehicle is a ship (isShipExact), don't mark as infantry; treat as NOT eligible for infantry branch.
                            if vehicleModel then
                                if isShipExact(vehicleModel) then
                                    eligible = false
                                else
                            ...
                            -- only on-foot infantry within INFANTRY_RANGE (do not treat seated ships as infantry)
                            local seat = hum.SeatPart
                            if not seat and dist <= INFANTRY_RANGE then
                                eligible = true
                            end
                        end
                        if eligible and dist < bestDist then
                            bestDist = dist
                            nearest = plr
                        end
                    end
...
-- VELOCITY & CACHE
-- Version 1.37 — adaptive smoothing inside orange zone
local function updatePositionCache(origin)
    local now = tick()
    local minAlpha = POS_SMOOTH_ALPHA or 0.35
    local maxAlpha = 0.9
    local cfg = MODE_CONFIG[mode] or MODE_CONFIG["General"]
    local orangeDist = cfg and cfg.TIER_ORANGE or MAX_CACHE_DISTANCE
        ...
        if ch then
            ...
            if pos then
                ...
                if dist <= MAX_CACHE_DISTANCE then
                    ...
                    -- ADAPTIVE ALPHA: starts changing inside orange zone,
                    -- linearly lerps from minAlpha -> maxAlpha as target approaches.
                    local alpha = minAlpha
                    if orangeDist and orangeDist > 0 and dist <= orangeDist then
                        local t = math.clamp((orangeDist - dist) / orangeDist, 0, 1)
                    ...
                    if prev and prev.pos and prev.time and now - prev.time > 0 then
                        ...
                        if prev.vel then
                            local sm = prev.vel * (1 - alpha) + rawVel * alpha
                            positionCache[plr] = { pos = pos, time = now, vel = sm }
                        else
                            positionCache[plr] = { pos = pos, time = now, vel = rawVel }
                        end
                    else
...
-- NOTE: per request, do NOT synthesize fallback speed for planes when stationary.
    ...
    if hum then
        ...
        if seat then
            ...
            if vehicle then
                ...
                -- DO NOT guess fallback for planes when stationary
            end
        end
    end
...
local function applyVehicleHighlight(vehicleModel, color)
    ...
    for _, v in ipairs(vehicleModel:GetDescendants()) do
        if v:IsA("BasePart") then table.insert(parts, { part = v, vol = math.abs(v.Size.X * v.Size.Y * v.Size.Z) }) end
    end
...
-- Detect vehicle model robustly for label/highlight
...
-- INFO LABELS (always update every frame)
local function updateInfoLabels(targetPlayer, distance, currentMode)
    if distance then pcall(function() distLabel.Text = string.format("Enemy distance: %.1f studs", distance) end)
    else pcall(function() distLabel.Text = "Enemy distance: N/A" end) end
    ...
    pcall(function()
        if hpVal ~= nil then
            vehicleLabel.Text = string.format("Vehicle: %s | HP: %d", vehicleModelName, hpVal)
        else
            vehicleLabel.Text = string.format("Vehicle: %s | HP: N/A", vehicleModelName)
        end
    end)

    local cfg = MODE_CONFIG[currentMode] or MODE_CONFIG["General"]
    if distance and distance < cfg.TIER_ORANGE then
        local _, tier = colorForDistance(distance, currentMode)
        pcall(function()
...
-- HIGHLIGHT RECREATION (only when needed)
local function setHighlightsIfNeeded(targetPlayer, distance, currentMode)
    ...
    if targetChanged then lastTarget = targetPlayer; lastColorTier = tier else lastColorTier = tier end
    ...
    if shouldRecreateHighlights then
        ...
        if vehicleModel then applyVehicleHighlight(vehicleModel, color) end
    else
        if vehicleModelChanged and vehicleModel then applyVehicleHighlight(vehicleModel, color) end
    end
...
local function stopLock()
    ...
    pcall(function() distLabel.Text = "Enemy distance: N/A"; velocityLabel.Text = "Velocity: N/A" end)
    isInDanger = false
    if isInCritical then isInCritical = false end
...
local function playPredictiveSoundIfNeeded(dist)
    if currentTarget ~= lastPredictTarget and dist <= (MODE_CONFIG[mode] and MODE_CONFIG[mode].PREDICTIVE_RANGE or 800) then
...
local function startLock()
    ...
    lockConn = RunService.RenderStepped:Connect(function()
        ...
        if mode == "General" then target, distVal = nearestEnemyWithVehicleHP(origin)
        elseif mode == "Anti-air" then target, distVal = nearestAntiAirTarget(origin)
        else target, distVal = nearestAntiShipTarget(origin) end
        ...
        if currentTarget then
            ...
            if targetPos and hum and hum.Health > 0 then
                ...
                -- update info labels every frame (distance, vehicle, HP, danger label)
                updateInfoLabels(currentTarget, dist, mode)
                ...
                if shouldPredict then playPredictiveSoundIfNeeded(dist) end
                ...
                -- REPLACEMENT: send predicted aim point to turret (no camera lock)
                pcall(function()
                    local aimPos = aimPoint
                    local args = {
                        "aim",
                        {
                            vector.create(aimPos.X, aimPos.Y, aimPos.Z)
                        }
                    }
                    game:GetService("ReplicatedStorage"):WaitForChild("Event"):FireServer(unpack(args))
                end)

                -- <<< ADD THIS: update tether visual (origin is 'origin', aim point is 'aimPoint') >>>
                -- <<< TETHER UPDATE: use vehicle center when seated, else use head-origin >>>
                pcall(function()
                    local camObj = Workspace and Workspace.CurrentCamera
                    -- detect if local player is seated and get the seat part if any
                    ...
                    -- consider free-cam OR being seated as valid states to show the tether
                    local camType = camObj and camObj.CameraType
                    local isFreeCam = (camType == Enum.CameraType.Custom) or seated
                    local showTether = isFreeCam and enabled and currentTarget ~= nil

                    -- determine origin for tether
                    local originForTether = origin

                    if seated and seatPart then
                        -- try to find the vehicle/model center:
                        local vehicleModel = seatPart:FindFirstAncestorOfClass("Model") or seatPart.Parent
                        local center = nil

                        if vehicleModel then
                            -- prefer model.PrimaryPart if set
                            if vehicleModel.PrimaryPart then
                                center = vehicleModel.PrimaryPart.Position
                            else
                                -- fallback: compute AABB center from all BaseParts in model
                                ...
                                for _, obj in ipairs(vehicleModel:GetDescendants()) do
                                    if obj:IsA("BasePart") then
                                        ...
                                        mins = Vector3.new(math.min(mins.X, ppos.X), math.min(mins.Y, ppos.Y), math.min(mins.Z, ppos.Z))
                                        maxs = Vector3.new(math.max(maxs.X, ppos.X), math.max(maxs.Y, ppos.Y), math.max(maxs.Z, ppos.Z))
                                    end
                                end
                                if mins.X ~= math.huge then
                                    center = (mins + maxs) / 2
                                end
                            end
                        end
                        ...
                        if center then
                            originForTether = center
                        else
                            -- fallback: use seatPart position if all else fails
                            originForTether = seatPart.Position
                        end
                    end

                    updateTether(CFrame.new(originForTether), aimPoint, showTether, dist, mode)
                end)
                ...
                -- HIT detection (SCRAPPED, THIS DOES NOTHING IN NO CAM-LOCKING VERSION) : only in Anti-air mode and inside ORANGE zone (includes critical)
                if mode == "Anti-air" and dist < cfg.TIER_ORANGE then
                    local vehicleModel = detectVehicleModelForPlayer(currentTarget)
                    local newHP = nil
                    if vehicleModel then
                        local okHP, hpObj = pcall(function() return vehicleModel:FindFirstChild("HP") end)
                        if okHP and hpObj and (hpObj:IsA("IntValue") or hpObj:IsA("NumberValue")) then newHP = hpObj.Value
                        else
                            local okAttr, attr = pcall(function() return vehicleModel:GetAttribute("HP") end)
                            if okAttr and type(attr) == "number" then newHP = attr end
                        end
                    end

                    local lastHP = lastKnownVehicleHPs[currentTarget]
                    if newHP ~= nil then
                        if lastHP == nil then
                            lastKnownVehicleHPs[currentTarget] = newHP
                        else
                        ...
                        lastKnownVehicleHPs[currentTarget] = nil
                    end
                end
                ...
                currentTarget = nil
                -- target lost -> hide tether
                pcall(function() updateTether(CFrame.new(origin), origin, false, nil, mode) end)
                clearCharHighlight(); clearVehicleHighlights()
                if crosshair then pcall(function() crosshair.ImageColor3 = Color3.new(1,1,1) end) end
                aiLabel.Visible = false; dangerLabel.Visible = false
                isInDanger = false
...
priorityBtn.MouseButton1Click:Connect(function()
    if mode ~= "Anti-ship" then priorityIndex = 1; priorityBtn.Text = "Priority: N/A"; return end
    priorityIndex = (priorityIndex % #SHIP_PRIORITY_ORDER) + 1
    priorityBtn.Text = "Priority: " .. SHIP_PRIORITY_ORDER[priorityIndex]
end)

bulletBtn.MouseButton1Click:Connect(function()
    if mode ~= "Anti-ship" then bulletTypeName = nil; bulletBtn.Text = "Bullet Type: N/A"; return end
...
player.CharacterAdded:Connect(function()
    ...
    aiLabel.Text = "Sentry Mode: " .. mode
    if enabled then
        -- [SCRAP] if crosshair then pcall(function() crosshair.ImageTransparency = 0.3 end) end
        aiLabel.Visible = true
    end
    lastKnownVehicleHPs = {}
end)
