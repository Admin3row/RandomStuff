...
-- v1.37.4 PATCH: SIMPLIFIED
local function createOrRefreshHitIndicator()
    -- recreate if missing or locked
    if not hitIndicator or not pcall(function() local _ = hitIndicator.Parent end) then
        if hitIndicator then pcall(function() hitIndicator:Destroy() end) end

        hitIndicator = Instance.new("ImageLabel")
        hitIndicator.Name = "AimHIT"
        hitIndicator.Rotation = 45
        hitIndicator.BorderSizePixel = 0
        hitIndicator.Image = "http://www.roblox.com/asset/?id=3874001308"
        hitIndicator.ImageColor3 = Color3.fromRGB(0,133,255)
        hitIndicator.ImageTransparency = 0.1
        hitIndicator.BackgroundTransparency = 1
        hitIndicator.Visible = false
        hitIndicator.ZIndex = 0

        -- fixed visual transform (centered in our ScreenGui by default)
        hitIndicator.AnchorPoint = Vector2.new(0.5, 0.5)
        hitIndicator.Position = UDim2.new(0.5, 0, 0.5, 0)
        hitIndicator.Size = UDim2.new(0.08, 0, 0.08, 0)
        hitIndicator.SizeConstraint = Enum.SizeConstraint.RelativeYY

        -- parent immediately to persistent GUI so it never gets destroyed on respawn
        pcall(function() hitIndicator.Parent = screenGui end)
    end

    if not hitSound or not pcall(function() local _ = hitSound.Parent end) then
        if hitSound then pcall(function() hitSound:Destroy() end) end
        hitSound = Instance.new("Sound")
        hitSound.Name = "AutoLockHitSound"
        hitSound.SoundId = HIT_SOUND_ID
        hitSound.Volume = HIT_VOLUME
        hitSound.Looped = false
    end

    -- ensure sound is parented to the indicator
    pcall(function() if hitSound and hitIndicator and hitSound.Parent ~= hitIndicator then hitSound.Parent = hitIndicator end end)
end

-- keep the indicator in our ScreenGui, but match anchor/position/sizeConstraint to the game's Aim (if available)
local function setHitIndicatorParentAndMatchCrosshair()
    if not hitIndicator then return end

    -- force parent to our persistent ScreenGui so it survives respawn
    if hitIndicator.Parent ~= screenGui then
        pcall(function() hitIndicator.Parent = screenGui end)
    end

    -- if the game's Aim exists and is in PlayerGui, match anchor/position & constraint (but keep fixed size)
    if crosshair and pcall(function() return crosshair:IsDescendantOf(playerGui) end) then
        pcall(function()
            hitIndicator.AnchorPoint = crosshair.AnchorPoint
            hitIndicator.Position = crosshair.Position
            hitIndicator.SizeConstraint = crosshair.SizeConstraint or Enum.SizeConstraint.RelativeYY
            hitIndicator.Size = UDim2.new(0.08, 0, 0.08, 0) -- keep fixed size
        end)
    else
        pcall(function()
            hitIndicator.AnchorPoint = Vector2.new(0.5, 0.5)
    ...
    -- ensure sound is parented
    if hitSound and hitSound.Parent ~= hitIndicator then
        pcall(function() hitSound.Parent = hitIndicator end)
    end
end

local function showHitIndicatorOnce()
    if not hitIndicator then return end
    pcall(function() hitIndicator.Visible = true end)
    if hitSound and hitSound.Parent then
        pcall(function() hitSound:Play() end)
    end
    task.delay(HIT_INDICATOR_DURATION, function()
        if hitIndicator and hitIndicator.Parent then
            pcall(function() hitIndicator.Visible = false end)
        end
    end)
end
...
warnSound.SoundId = WARNING_SOUND_ID
warnSound.Volume = 2
...
local function refreshCrosshair()
    ...
    -- create / refresh hit indicator and parent it to same ScreenGui as Aim (crosshair.Parent) if possible
...
RunService.RenderStepped:Connect(function()
    ...
    if crosshair then
        ...
        -- ensure our hit indicator follows the Aim's transform every frame
...
playerGui.DescendantAdded:Connect(function(desc)
    if desc.Name == "Aim" and (desc:IsA("ImageLabel") or desc:IsA("ImageButton")) then
        ...
        centerCrosshair(crosshair)
        pcall(function() crosshair.ImageTransparency = enabled and 0.3 or 0.6 end)
        -- when the game's Aim is created (often in ScreenGui2), parent our hitIndicator to the same ScreenGui and match it
        createOrRefreshHitIndicator()
        setHitIndicatorParentAndMatchCrosshair()
    end
end)
...
local function startLock()
    ...
    -- ensure hit indicator exists and parent matches current crosshair
    createOrRefreshHitIndicator()
    setHitIndicatorParentAndMatchCrosshair()
    ...
    lockConn = RunService.RenderStepped:Connect(function()
        ...
        if currentTarget then
            ...
            if targetPos and hum and hum.Health > 0 then
                ...
                -- HIT detection: only in Anti-air mode and inside ORANGE zone (includes critical)
